...
import first.common.util.exception.JsonParseExecption;
...
public final class JsonParser {
	...
	static String mapKey(CharTokenizer token) {
		...
		if (token.read() == '"') {
			...
			throw new JsonParseExecption(
					"Map's 'VALUE' Parse Error. Not found ':' characterset. (" + token.debugText() + ")");
		}
	...
	static Var map(CharTokenizer token) {
		...
		while (token.hasNext()) {
			...
			if (isEmpty) {
				...
				if (token.next() == ',') {
					...
					throw new JsonParseExecption(
							"Map Next Item Character is ',' expected. (" + token.debugText() + ")"); // 맵은
																										// {}
																										// 사이에
																										// 있어야한다.
				}
		...
		throw new JsonParseExecption("Map close Character is '}' (" + token.debugText() + ")"); // 맵은
																								// {}
																								// 사이에
																								// 있어야한다.
	...
	static Var list(CharTokenizer token) {
		...
		while (token.hasNext()) {
			...
			if (isEmpty) {
				...
				token.skipWhiteSpace();
				if (token.next() != ',') {
					throw new JsonParseExecption("Array next Item Character is ',' (" + token.debugText() + ")"); // 아이템
																													// 사이에는
																													// ,
																													// 가
																													// 있어야
		...
		throw new JsonParseExecption("Array close Character is ']' (" + token.debugText() + ")"); // 배열은
																									// []
																									// 사이에
																									// 있어야한다.
	}
	...
	/**
	 * 문자열 데이터를 파싱한다. 문자열은 " " 사이의 값을 의미한다. 이스케입처리는 ( \n \r \t \\ \" )
	 * 
	 * @param token
	 * @return
	 */
	static Var string(CharTokenizer token) {
		...
		throw new JsonParseExecption("String close Character is '\"' (" + token.debugText() + ")"); // 문자열은
																									// "
																									// 로
																									// 닫혀야한다.
	}
	 ...
	 * 숫자형 데이터를 파싱한다. <br/>
	 * 
	 * 마지막 token 위치에서 숫자형 데이터인 부분을 찾는다. 이때, token의 커서는 자동으로 이동된다. 데이터가 없을경우
	 * null을 리턴한다.
	 * 
	 * @param token
	 * @return
	...
	private static Character escape(CharTokenizer token) {
		if (token.hasNext() == false || token.read() != '\\')
			...
			if (token.hasNext() == false)
				throw new JsonParseExecption("escape next token is empty.(" + token.debugText() + ")"); // 이스케입하고
																										// 뒤에
																										// 글자가
																										// 없다.
		}
		...
		if (token.next() != 'u') {
			throw new JsonParseExecption(
					"escape parse error. [expected= \\t, \\f, \\n, \\t, \\r, \\\\, \\\", \\u0000 ~ \\uFFFF] ("
							+ token.debugText() + ")"); // escape이 잘못되었다.
		}
		...
		for (int i = 0; i < 4; i++) {
			if (token.hasNext())
				...
				throw new JsonParseExecption("unicode escape short length. (expected: 4digit number. \\u" + sb
						+ ", length=" + sb.length() + "/4)] (" + token.debugText() + ")"); // u형태의
																							// 유니코드표현의
																							// 숫자가
																							// 짧다.
		}
		...
		try {
			...
			throw new JsonParseExecption("unicode escape is not Number. (" + token.debugText() + ")"); // escape
																										// 숫자형값이
																										// 아님.
		}
	}
